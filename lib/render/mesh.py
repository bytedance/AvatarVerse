...
def auto_normals(imesh):
    ...
    v_nrm = v_nrm.index_add(0, i0, face_normals)
    v_nrm = v_nrm.index_add(0, i1, face_normals)
    v_nrm = v_nrm.index_add(0, i2, face_normals)
...
######################################################################################
# Remove components with small number of faces
# Reference: https://github.com/threestudio-project/threestudio/blob/52925f8bf02232bf750b6e5c6e3a6af1bc670678/threestudio/models/mesh.py#L31
######################################################################################
def remove_outlier(imesh, outlier_n_faces_threshold) -> Mesh:
    ...
    mesh = trimesh.Trimesh(
        vertices=imesh.v_pos.detach().cpu().numpy(),
        faces=imesh.t_pos_idx.detach().cpu().numpy(),
    )
    ...
    v_pos = torch.from_numpy(mesh.vertices).to(imesh.v_pos)
    t_pos_idx = torch.from_numpy(mesh.faces).to(imesh.t_pos_idx)
