...
def sdf_reg_loss(sdf, all_edges):
    """
        This reduces the number of sign flips and simplifies the surface, penalizing internal geometry or floaters.
        ref: https://github.com/NVlabs/nvdiffrec/blob/3faedd23813ff6a34fd69d4d5b466eb0485c70e1/geometry/dmtet.py#L151
    """
...
class DMTetGeometry:
    def __init__(
            self, grid_res=64, scale=torch.tensor([2., 2, 2]), device='cuda', args=None):
        self.grid_res = grid_res
        self.device = device
        self.args = args

        scale_np = scale.cpu().numpy()
        valid_l_h_rates = [1, 2, 3, 5, 7]
        l_h_rate = min(max(int(scale_np[1] / max(scale_np[0], scale_np[2])), valid_l_h_rates[0]), valid_l_h_rates[-1])

        while l_h_rate not in valid_l_h_rates:
            l_h_rate -= 1
        print('dmtet: using dmtet template tet_1_%d_1.npz'% l_h_rate)
        # tets = np.load('data/tets/%d_compress.npz' % (grid_res))
        tets = np.load('data/tets/tet_1_%d_1.npz' % l_h_rate)
        ...
        # else:
        self.verts = self.verts * scale[1]
        # self.indices = torch.from_numpy(tets['tets']).long().to(self.device)
        self.indices = torch.from_numpy(tets['indices']).long().to(self.device)
